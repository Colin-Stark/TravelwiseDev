...
export default function SearchFlightStep2(props) {
    ...
    const initialValues = {
            ...
            departure_city : props.initialData["departure_city"] ? props.initialData["departure_city"] : "",
            departure_id : props.initialData["departure_id"] ? props.initialData["departure_id"] : "",
        };
    
    const [country, setCountry] = useState(initialValues.departure_country);
    const [city, setCity] = useState(initialValues.departure_city);
    const [selectedAirport, setSelectedAirport] = useState(initialValues.departure_id);

    const { Formik } = formik;
    const schema = yup.object().shape({
        country: yup.string(),
        city: yup.string(),
        departure_id: yup.string(),
    });
    ...
    useEffect(() => {
        ...
        if(initialValues.departure_country) {
            loadCityData(initialValues.departure_country);

            if(initialValues.departure_city) {
                loadAirportData(initialValues.departure_city);
            }
        }

    }, [setIsBlocked, loadCountryData, loadCityData, loadAirportData, initialValues.departure_country, initialValues.departure_city]);

    async function handleSubmit(values) {
        setWarning(""); // Clear previous warnings

        if(!values.departure_id) {
    ...
    return (
        ...
        <Form className="mx-sm-2 mx-md-4" onSubmit={handleSubmit} as={formik.Form}>
            <h3 className="text-center">Departure Details</h3>
            <p className="text-center">Select the airport to depart from</p>
                        ...
                        <Card.Body>
                            ...
                            <Row className="gy-3">
                            {
                                airports?.length > 0 ? airports.map((airport, index) => (
                                    <Col md={12} lg={6} key={index} onClick={()=> {values.departure_id = airport.IATA; setSelectedAirport(airport.IATA);}}>
                                            ...
                                            <Card.Body>
                                                    <div className='d-flex justify-content-between align-items-center'>
                                                        <Form.Check 
                                                            ...
                                                            onChange={() => values.departure_id = airport.IATA}
                                                        />
